    ...
    for data in all_data_json.values():
        ...
        DU.save_data_to_json(
            os.path.join(
                ...
                "p%s" % str(p_num),
                "m%s" % str(m_num),
                "data%s.json" % str(int(dir_count[p_num - 1][m_num - 1])),
            ),
            data,
        )
...
def main():
    ...
    for i in range(p_upper_limit):
        for j in range(m_upper_limit):
            all_sample_list = DU.sample_dir_json(
                each_grid_num, grid_data_loc % (str(i + 1), str(j + 1))
            )
    ...
    DU.save_data_to_json(
        os.path.join(save_grid_loc, "ori_data", "all_data.json"), all_data_ditc
    )
    ...
    start = time.time()
    with open(
        os.path.join(os.path.join(save_grid_loc, "package_data"), "dataset.pkl"), "wb"
    ) as f:
