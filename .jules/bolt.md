...
## 2024-05-01 - Optimize solver for state equation using bicgstab
**Learning:** `scipy.sparse.linalg.lsqr` is very robust against singular or ill-conditioned matrices but can be quite slow for iterative probability calculations inside a deep loop. Conversely, `scipy.sparse.linalg.bicgstab` is a fast iterative solver that works well for the square matrices typically generated by SPN state equations.
**Action:** When solving the state equation system $A x = b$, attempt to use `bicgstab(A, b)` first if $A$ is square. If it fails to converge (`info != 0`), gracefully fallback to `lsqr(A, b)`. Furthermore, when flattening the resulting probabilities matrix, use `.ravel()` instead of `.flatten()` to prevent intermediate array allocations. This combination drops the generation time drastically without losing correctness.

## 2026-06-08 - Use lazily initialized thread-local scratchpads for tight loops
**Learning:** During parallel generation (via Joblib/Loky), SPN data structures vary wildly in size per step, resulting in continuous `np.empty` and `np.zeros` allocations inside tight loops like `_bfs_core` and `_compute_state_equation_numba`. Numba compiles these inner loop routines down to rapid C-level instructions, but array allocation and garbage collection churn remains a massive bottleneck (taking ~3-5 seconds out of every 15). Pre-allocating exact-size buffers per SPN sample outside of the loops requires constant re-allocation due to shape mismatches or incurs pickling overhead.
**Action:** Lazily initialize process/thread-local max-capacity 1D/2D numpy array scratchpads via `threading.local()` (or global caches) for each worker. Allocate arrays slightly larger than typical maximums (e.g. `num_vertices * 2`) and dynamically grow them if exceeded. Pass these buffers into Numba JIT functions to drastically cut allocation time and GC churn for performance-critical loops without breaking Numba's strict type caching.
