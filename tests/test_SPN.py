...
def test_is_connected_isolated_transition():
    ...
    # T2 is isolated
    petri_net_matrix = np.array([
        [1, 0, 0, 0, 0, 1, 1], # P1: T1 -> P1, T3 -> P1
        [0, 0, 1, 0, 0, 1, 0]  # P2: T3 -> P2
    ])
    assert is_connected(petri_net_matrix) is False
...
def test_compute_state_equation():
    ...
    #  [2, 0],
    #  [1, 1]]
    assert np.allclose(state_matrix.toarray(), np.array([[-2., 0.], [2., 0.], [1., 1.]]))
    assert np.allclose(target_vector, np.array([0., 0., 1.]))
...
def test_solve_for_steady_state_no_solution():
    ...
    state_matrix = csc_array(np.array([[0., 0.], [0., 0.]]))
...
def test_generate_stochastic_net_task():
    ...
    arc_transitions = [0]
    num_transitions = 1
    _, _, markings, _, success = generate_stochastic_net_task(
...
def test_compute_state_equation_numba():
    ...
    expected_matrix = np.array([[-2., 0.], [2., 0.], [1., 1.]])
