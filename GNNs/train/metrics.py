...
def ErrorRate(scores, targets):
    errra = torch.abs(scores - targets) / targets
    errra = torch.mean(errra, dim=0)[0]
    return errra
...
def accuracy_SBM(scores, targets):
    ...
    for r in range(nb_classes):
        ...
        if cluster.shape[0] != 0:
            ...
            pr_classes[r] = 0.0
    acc = 100.0 * np.sum(pr_classes) / float(nb_non_empty_classes)
    return acc
